...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(Faker $faker)
    {
        ...
        /*factory(Auction::class, 400)->create();*/
        /*if (env('APP_URL') !== 'http://adomino.net') {*/
        if (false) {
            for($i = 0; $i < 400; $i++){

                $domain = Domain::all()->random()->domain;
               /* while (Auction::whereDomain($domain)->get()){
                    $domain = Domain::all()->random()->domain;
                }*/
                ...
                $day_interval = $faker->numberBetween(7,14);
                $start_date = Carbon::now()->addDay($day_interval);
                $end_date = Carbon::now()->addDay(($day_interval+$days));
