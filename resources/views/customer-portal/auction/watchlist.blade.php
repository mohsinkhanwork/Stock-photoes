            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Meine Watchlist</h3>
                    </div>
                    <div class="card-body">
                        <div class="alert-block col-12"> </div>
                        {{--<p class="mb-4"> Hier finden Sie alle Auktionen die Sie in die Watchlist aufgenommen haben. Falls es bei diesen Auktionen Änderungen geben sollte, erhalten Sie jeweils ein gesondertes E-Mail. Somit können Sie die für Sie interessanten Auktionen überwachen ohne selbst bieten zu müssen. </p>
                        --}}<div class="table-holder ">
                            <ul class="menu">
                                <li><a class="h4 {{$type == 'planned' ? 'active' : ''}}" href="{{route('customer.auction.watchlist', 'planned')}}"> <span>Geplante</span> Auktionen (<span class="planned_count {{$type == 'planned' ? 'text-dark' : '' }} ">{{$planned}}</span>) </a></li>
                                <li><a class="h4 {{$type == 'active' ? 'active' : ''}}" href="{{route('customer.auction.watchlist', 'active')}}"><span>Aktive</span> Auktionen (<span class="active_count  {{$type == 'active' ? 'text-dark' : '' }}">{{$active}}</span>)</a></li>
                                <li><a class="h4 {{$type == 'sold' ? 'active' : ''}}" href="{{route('customer.auction.watchlist', 'sold')}}"><span>Verkaufte </span> Domains (<span class="sold_count  {{$type == 'sold' ? 'text-dark' : '' }}">{{$sold}}</span>)</a></li>
                                <li><a class="h4 {{$type == 'finish' ? 'active' : ''}}" href="{{route('customer.auction.watchlist', 'finish')}}"><span>Beendete </span> Auktionen (<span class="finish_count  {{$type == 'finish' ? 'text-dark' : '' }}">{{$finished}}</span>)</a></li>
                            </ul>
                            @if ($type == 'planned')
                                <p class="mb-4">Hier finden Sie alle geplanten Auktionen die Sie in die Watchlist aufgenommen haben. Falls es bei diesen Auktionen Änderungen geben sollte, erhalten Sie jeweils ein gesondertes E-Mail. Somit können Sie die für Sie interessanten Auktionen überwachen ohne selbst bieten zu müssen. </p>
                            @elseif ($type == 'active')
                                <p class="mb-4">Hier finden Sie alle aktiven Auktionen die Sie in die Watchlist aufgenommen haben. Falls es bei diesen Auktionen Änderungen geben sollte, erhalten Sie jeweils ein gesondertes E-Mail. Somit können Sie die für Sie interessanten Auktionen überwachen ohne selbst bieten zu müssen.</p>
                            @elseif ($type == 'sold')
                                <p class="mb-4">Hier finden Sie maximal 10  verkaufte Domains die Sie in die Watchlist aufgenommen haben und die nicht älter als 30 Tage alt sind.</p>
                            @elseif ($type == 'finish')
                                <p class="mb-4">Hier finden Sie maximal 10  beendete Auktionen die Sie in die Watchlist aufgenommen und die nicht älter als 30 Tage alt sind. </p>
                            @endif
                            <table class="table auction-listing auction-table">
                                <thead>
                                    <tr class="table-active">
                                        ...
                                        <th width="6%" class="text-center no-sort">Watch</th>
                                    </tr>
                                </thead>
                                <tbody> </tbody>
                            </table>
    ...
    <script>
        ...
        $(document).ready(function (){
            ...
            table = $('.auction-table').DataTable({
                    ...
                    ajax: {
                        {{-- url: '{{ route('customer.auctions.type','watchlist') }}', --}}
                        url: '{{ route('customer.auction.watchlist', $type) }}',
                    },
                    columns: [
                        ...
                        {data: 'actual', name: 'actual', class: 'text-right  text-success' },
                        {data: 'discount', name: 'discount', class: 'text-right text-success'  },
